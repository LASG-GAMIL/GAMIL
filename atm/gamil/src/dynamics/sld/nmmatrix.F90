...
!-----------------------------------------------------------------------
!
! $Id: nmmatrix.F90,v 1.2.42.1 2002/06/15 13:48:27 erik Exp $
! $Author: erik $
!
!-----------------------------------------------------------------------
...
!
  ...
  if (int(work(1)) .gt. lwork) then
     write(6,*) 'WARNING in NMMATRIX:'
     write(6,*) 'optimal dimension of "work" array is: '
     write(6,*) int(work(1))
     call endrun
  endif
!
...
!
  ...
  if (lneg) then
     write(6,*) 'ERROR in NMMATRIX: negative eigenvalue(s)'
     call endrun
  endif
  if (lcomp) then
     write(6,*) 'ERROR in NMMATRIX: complex eigenvalue(s)'
     call endrun
  endif
  if (lsame) then
     write(6,*) 'ERROR in NMMATRIX: non-distinct eigenvalue(s)'
     call endrun
  endif
!
...
!
  ...
  if (lnonzero) then
     write(6,*) 'ERROR in NMMATRIX:'
     write(6,*) 'Y(T)X has non-zero off-diagonal elements'
     call endrun
  endif
!
  return
