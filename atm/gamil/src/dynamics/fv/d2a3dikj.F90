...
! !INTERFACE:
      ...
      subroutine d2a3dikj(u,      v,      ua,     va,     im,            &
                          jm,     ktot,   jfirst, jlast,  ng_d,          &
                          ngus,   ngun,   ngvs,   ngvn,  ifirst,         &
                          ilast,  coslon, sinlon)
...
! !INPUT PARAMETERS:
      ...
      integer, intent(in)  :: ngus    ! ghost latitudes for U south
      integer, intent(in)  :: ngun    ! ghost latitudes for U north
      integer, intent(in)  :: ngvs    ! ghost latitudes for V south
      integer, intent(in)  :: ngvn    ! ghost latitudes for V north
      integer, intent(in)  :: ifirst  ! longitude strip start
      integer, intent(in)  :: ilast   ! longitude strip finish
      real(r8), intent(inout) :: u(ifirst:ilast,jfirst-ngus:jlast+ngun,ktot) ! U-Wind
...
! !INPUT/OUTPUT PARAMETERS:
      real(r8), intent(inout) :: ua(ifirst:ilast,ktot,jfirst:jlast) ! U-Wind
      real(r8), intent(inout) :: va(ifirst:ilast,ktot,jfirst:jlast) ! V-Wind
...
!BOC
      ...
      integer imh, i, j, k, itot, jtot, ltot, lbegin, lend, ik, jend
...
! Set ua on A-grid
      ...
      call mp_recv3d( iam+nprxy_x, im, jm, ktot,                         &
                      ifirst, ilast, jfirst-ngus, jlast+ngun, 1, ktot,   &
                      ifirst, ilast, jlast+1, jlast+1, 1, ktot, u )
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,ktot
        do j=jfirst, jend
          do i=ifirst,ilast
            ua(i,k,j) = 0.5*(u(i,j,k) + u(i,j+1,k))
          enddo
        enddo
      enddo
...
!$omp  parallel do private(i,j,k)
      ...
      do k=1,ktot
         do j=jfirst, jlast
            do i=ifirst,ilast-1
               va(i,k,j) = 0.5*(v(i,j,k) + v(i+1,j,k))
            enddo
            va(ilast,k,j) = 0.5*(v(ilast,j,k) + veast(j,k))
         enddo
      enddo
...
!$omp  parallel do private(i,k)
         do k = 1,ktot
            do i=ifirst,ilast
               uvaloc(i,k,1) = ua(i,k,2)
               uvaloc(i,k,2) = va(i,k,2)
               uvaglob(i,k,1) = ua(i,k,2)
               uvaglob(i,k,2) = va(i,k,2)
            enddo
         enddo
         lbegin = 1
         lend = 2
...
!$omp  parallel do private(i,k)
         do k = 1,ktot
            do i=ifirst,ilast
               uvaloc(i,k,3) = ua(i,k,jm-1)
               uvaloc(i,k,4) = va(i,k,jm-1)
               uvaglob(i,k,3) = ua(i,k,jm-1)
               uvaglob(i,k,4) = va(i,k,jm-1)
            enddo
         enddo
         lbegin = 3
         lend = 4
