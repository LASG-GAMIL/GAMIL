...
! !LOCAL VARIABLES:
      ...
      INTEGER BlockLen, Ipe, Comm_1, Comm_2
      INTEGER myid_1, myid_2, npr_1, npr_2, size_1, size_2, rank_1, rank_2
      LOGICAL Passed
...
! For the 2D and 3D decompositions
      ...
      REAL, ALLOCATABLE :: Aglobal2d(:), Bglobal2d(:)
      REAL, ALLOCATABLE :: Aglobal3d(:), Bglobal3d(:)
      REAL, ALLOCATABLE :: Rsemiglobal3d(:)
      REAL, ALLOCATABLE :: Rlocal2d(:), Rlocal3d(:)
      REAL, ALLOCATABLE :: Rtmp(:), Rlocal(:), Rglobal(:)
      REAL :: Scalar, Q, Array1D(Im), Array2D(Im,Jm), Array3D(Im,Jm,Km)
...
!
      ...
      IF ( GID == 0 ) THEN
        IF ( SUM(Aglobal3D-Bglobal3D) /= 0.0 ) THEN
...
!
      ...
      IF ( GID == 0 ) THEN
        IF ( SUM( Aglobal2d - Bglobal2d ) /= 0.0 .OR.                     &
     &       SUM( Aglobal3d - Bglobal3d ) /= 0.0 )  THEN
...
!
        ...
        IF ( SUM( Rglobal - Rlocal ) /= 0.0 ) THEN
