#include <misc.h>
#include <params.h>
subroutine sltint(kdim    ,jcen    ,fb      ,lam     ,rdphi   , &
...
!-----------------------------------------------------------------------
!
! $Id: sltint.F90,v 1.3.42.2 2002/06/21 05:37:40 erik Exp $
! $Author: erik $
!
!-----------------------------------------------------------------------
...
!
        ...
        do jdpval = jmin,jmax
           do k=1,plev
              call wheneq(nlon    ,jdp(1,k),1       ,jdpval  , &
                          indx    ,nval    )
              icount = icount + nval
!
...
!
        ...
        if (icount.ne.nlon*plev) then
           write(*,*)'SLTINT:  Did not complete computations for all departure points'
           call endrun
        end if
!
...
!
     if( limdrv ) then
        ...
        do kdpval = 1,kdimm1
           do k=1,plev
              call wheneq(nlon    ,kdp(1,k),1       ,kdpval  , &
                          indx    ,nval    )
              icount = icount + nval
              do ii = 1,nval
                 i = indx(ii)
...
!
        ...
        if (icount.ne.nlon*plev) then
           write(6,*)'SLTINT:  Did not complete computations for all departure points'
           call endrun
        endif
     endif
!
...
!
     ...
     if(limdrh .or. limdrv) then
        write(6,*) 'SLTINT:  ERROR:  this routine does not provide '
        write(6,*) 'shape preserving capability for vertical-only'
        write(6,*) ' interpolation'
        call endrun
     end if
...
!!!!!                  + tmpbot            *dhb(i,k)
     ...
     write(6,*) 'SLTINT:  Error: must specify at least one of "lhr', &
          'zint" or "lvrtint" to be ".true."'
     call endrun
  end if
!
  return
